...
const getFormatData = (archivesData, summaryInfo, stepCategory) => {
...
const ClxCheesecakeSummaryTable = ({
  technology,
  feature,
  filters,
  ...
  useEffect(() => {
    if (archivesInfo) {
      ...
      const data = getFormatData(archivesInfo, summaryInfo, "model")
      const newHeader = getHeader(header, summaryInfo)
      const newTableBody = getTableBody(data, header)
      setTableState({
        body: newTableBody,
        header: newHeader,
        isProcessing: false,
  ...
  console.log("Rendering ClxCheesecakeSummaryTable")
  return (
    <>
...
export default React.memo(ClxCheesecakeSummaryTable)
