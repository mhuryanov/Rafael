import { CDF_TABLE, FITNESS_FEATURES, KPI_NAMES } from "../../../utilities/constants"
...
export const getAggregatedData = (
  ...
  const { kpiColumn: categoryColumn } = getCdfTable(technology, feature)
  const kpiColumns = getKpiColumns(selectedKpi)
  let plotlyData = []
  ...
  const getCustomFilterOptions = (archiveData) => {
    Object.keys(customFilters).forEach((colName) => {
      if (colName in archiveData) {
        const values = archiveData[colName]
        addToObject(customFilterOptions, colName, values)
      }
    })
  }
  ...
  archives
    ...
    .forEach((archive) => {
      ...
      if (!isEmpty(archiveData)) {
        ...
        getCustomFilterOptions(archiveData)
        categories.forEach((category, idx) => {
          if (category === selectedCategory && equalsCustomFilters(customFilters, archiveData, idx)) {
            const [offset, deviceBuildType] = helper(deviceType, buildTrain)
            kpiColumns.forEach((kpiColumn, colIdx) => {
              const error = archiveData[kpiColumn][idx]
              if (typeof error === "string") {
                // string of comma-separated percentiles
                const errorPercentiles = error.split(",").map((entry) => Number(entry))
                if (errorPercentiles.length === 101) {
                  const percentiles = getPercentiles(technology, feature)
                  percentiles.forEach((percentile, j) => {
                    const subOffset = BASE_OFFSET / (1 + percentiles.length)
                    const label = `${percentile}%`
                    const dataPoint = {
                      name: deviceBuildType,
                      placement: subOffset * (j + 1) + offset,
                      value: errorPercentiles[percentile],
                      archive,
                      label,
                    }
                    if (!excludedIds.includes(archiveId)) {
                ...
                // individual value
                const subOffset = BASE_OFFSET / (1 + kpiColumns.length)
                const label = getKpiName(kpiColumn)
                const dataPoint = {
                  name: deviceBuildType,
                  placement: subOffset * (colIdx + 1) + offset,
                  value: error,
                  archive,
                  label,
                }
  ...
  Object.entries(customFilterOptions).forEach(([filterName, filterOptions]) => {
    customFilterOptions[filterName] = _.uniq(filterOptions).sort()
  })
        ...
        const [entryKpi] = kpis
          .filter((kpi) => {
            return (
              ...
              "environment" in kpi.meta &&
              kpi.meta.environment.toUpperCase() === signalEnv.toUpperCase() &&
              ("device type" in kpi.meta ? kpi.meta["device type"].toUpperCase() === deviceType.toUpperCase() : true)
            )
          })
          .sort((a, b) => (a.date < b.date ? 1 : -1))
        return entryKpi
...
export const getCategories = (technology, feature, selectedKpi) => {
  switch (true) {
    case technology === "GNSS" && feature === "TTFF":
      return ["ttff_gpssa", "h_err_gpssa", "v_err_gpssa"]
    case technology === "GNSS":
      return [...Object.keys(KPI_NAMES.GNSS), "odometer_distance_error_rate"]
    case technology === "E911" && selectedKpi === "Percent of Error <":
      return [
        "WiFi SLAM Altitude Error",
        "DEMOnlyBaro Altitude Error",
        "RefBasedBaro Altitude Error",
        "CPI-WiFi Altitude Error",
        "Injected Altitude Error",
        "NILR Altitude Error",
      ]
    case technology === "E911":
      return [
        "WiFi SLAM Altitude Error",
        "WiFi SLAM Horizontal Error",
        "DEMOnlyBaro Altitude Error",
        "RefBasedBaro Altitude Error",
        "CPI-WiFi Altitude Error",
        "CPI-WiFi Horizontal Error",
        "Injected Altitude Error",
        "Injected Horizontal Error",
        "NILR Altitude Error",
        "NILR Horizontal Error",
      ]
    default:
      return []
  }
...
export const getCustomFilters = (technology, feature) => {
  switch (true) {
    case technology === "GNSS" && feature !== "TTFF":
      return { signal_env: "Benign", source: "CL Pos" }
    case technology === "GNSS" && feature === "TTFF":
      return { signal_env: "Benign" }
    default:
      return {}
  }
}
